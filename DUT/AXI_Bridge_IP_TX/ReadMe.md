...
## 7. Reset and flush behavior (top-level)
### 7.1 Reset
- On `rst_ni=0`, submodules reset their internal state; ingress deasserts events; serializer and stats reset according to their RTL.

### 7.2 Flush / clear
- FIFO flush is controlled by serializer via `fifo_clear` (wired to `fifo_sync.clear_i`).
- The precise conditions that trigger clear/drop are to be documented from serializer RTL.

---

## 8. Verification recommendations (current)
These are minimal tests to validate ingress behavior before serializer details are finalized:

1) **Basic acceptance:** drive `bridge_enable=1`, FIFO empty, and send beats with `tvalid=1` and varying `tkeep`; confirm `s_axis_tready=1` and `fifo_push` only on handshake.

2) **Almost-full backpressure:** sweep `fifo_level` around `tx_fifo_afull_thr` and confirm `s_axis_tready` deasserts when `fifo_level >= thr_trunc`.

3) **Strict tkeep checks:** with `strict_tkeep_en=1`, drive patterns expected to fail `tkeep_illegal_f_fixed()` and confirm pulse on accepted beats only.

4) **Overflow pulse (sanity):** (optional) force `fifo_full=1` while also forcing an accepted push path in simulation, confirm `ev_err_overflow_tx_pulse` can fire if conditions are met.
...
| Version | Date | Notes |
|---|---|---|
| 0.1 | (today) | Initial spec draft + ingress finalized from RTL |
